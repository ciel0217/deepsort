...
class KalmanFilter(object):
    def __init__(self):
        ...
        self._motion_mat = np.eye(ndim*2, 2*ndim)
        for i in range(ndim):
    ...
    def initiate(self,measurement):
        ...
        self.w = np.zeros(self.particle_num)
    ...
    def update(self,mean,covariance,measurement):
        ...
        for i in range(self.particle_num):
            ...
            me += self.particle[i,:]*self.w[i]
            
        mean = me
        #print(mean)
        for i in range(self.particle_num):
            P += np.diag((self.particle[i,:]-mean) * ((self.particle[i,:]-mean).T )*  self.w[i])
            
        for i in range(self.particle_num):
            self.particle[i,:] = mean + np.dot((np.random.randn(8,8)),(np.sqrt(np.diag(P))))
        self.particle = abs(self.particle)
        print(self.particle)
        print(measurement)
        new_mean = mean
